  ...
  <table class="table table-bordered rounded table-sm">
    <thead >
      <tr >
        ...
        <th class="bg-dark bg-gradient text-white">Category Name</th>
    ...
    <tbody>
        ...
        <tr>
            <td>{{++$sl}}</td>
            {{-- <td style="max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ $product->category->name }}</td> --}}
            <td>{{$product->category->name}}</td>
            <td>
                <img style="width:120px;" src="{{asset('images/products/'.$product->image)}}" alt="">
            </td>
            <td style="max-width: 270px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{$product->description}}</td>

            <td>{{$product->price}}</td>
            <td>
                {{-- <a href="{{route('product_view',$product->id)}}" class="btn btn-info btn-sm text-white">View</a> --}}
                <a href="{{route('product_edit',$product->id)}}" class="btn btn-primary btn-sm text-white">Edit</a>
                <a href="{{route('product_delete',$product->id)}}" onclick="return confirm('Are you sure to delete?')" class="btn btn-danger btn-sm text-white">Delete</a>
...
<a href="{{route('product_create')}}" class="btn btn-primary mb-3 ">Add Product</a>
